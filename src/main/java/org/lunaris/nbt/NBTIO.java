...
import org.lunaris.nbt.tag.Tag;
...
public class NBTIO {
     ...
     */
    public static CompoundTag putItemHelper(ItemStack item) {
        return putItemHelper(item, null);
    }

    public static CompoundTag putItemHelper(ItemStack item, Integer slot) {
        CompoundTag tag = new CompoundTag(null)
                .putShort("id", item.getType().getId())
                .putByte("Count", item.getAmount())
                .putShort("Damage", item.getData());
        if (slot != null) {
            tag.putByte("Slot", slot);
        }

        if (item.hasCompoundTag()) {
            tag.putCompound("tag", item.getCompoundTag());
        }

        return tag;
    }

    public static ItemStack getItemHelper(CompoundTag tag) {
        ...
        try {
            ...
            item.setAmount(tag.getByte("Count"));
        }
